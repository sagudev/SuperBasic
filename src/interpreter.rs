...
impl Interpreter {
    ...
    fn exec(&mut self, node: &AST) -> Result<Execute> {
        ...
        /* println!("exec     : {:?}: {}", node.line_num, node);
        println!("self.vars     : {:?}", self.vars);
        println!("self.md_vars     : {:?}", self.md_vars);
        println!("self.fn_vars     : {:?}", self.fn_vars);
        println!("self.fn_defs     : {:?}", self.fn_defs);
        println!("self.data     : {:?}", self.data);
        println!("self.fornext     : {:?}", self.fornext);
        println!("self.returns     : {:?}", self.returns);
        println!("self.curr_data     : {:?}", self.curr_data);
        println!("self.curr_stmt     : {:?}", self.curr_stmt);
        println!("self.line_num     : {:?}", self.line_num); */
    ...
    fn exec_exit(&self) -> Result<Execute> {
        std::process::exit(0);
    }
    ...
    pub fn runl(&mut self, ast: &AST) -> Result<()> {
        if ast.token.id != TID::ROOT {
            debug!("AST must always start with a root node!");
            return Err(Error::RuntimeError(
                self.line_num,
        ...
        while curr < ast.children.len() {
            self.curr_stmt = curr as u32;
            //println!("EXEC  {:?}", ast.children[curr]);
            match self.exec(&ast.children[curr])? {
                Execute::Stop => break,
                Execute::Next => curr = curr + 1,
                Execute::NextLine => loop {
